...
class AmpBluetoothService {
  ...
  // 扫描设备 - 改进版本，使用平台兼容参数
  Future<Stream<List<ScanResult>>> scanDevices({Duration? timeout}) async {
    ...
    await FlutterBluePlus.startScan(
      ...
      androidScanMode: AndroidScanMode.lowLatency, // 低延迟扫描模式，提高发现率
    );
  ...
  // 开始蓝牙扫描 - 符合规范的实现
  Future<void> startBluetoothScan({Duration timeout = const Duration(seconds: 20)}) async {
    print("🔄 开始蓝牙扫描流程");
    ...
    try {
      ...
      print('✅ 扫描已启动，持续 ${timeout.inSeconds} 秒');
      
      // 监听扫描结果
      _scanResultsSubscription = FlutterBluePlus.scanResults.listen((results) {
        print('🔍 扫描到 ${results.length} 个设备');
        for (var result in results) {
          print('📱 设备名称: "${result.device.name}", ID: ${result.device.id}, RSSI: ${result.rssi}');
        }
        // 这里可以将结果通过回调或流传递给UI层
      });
    ...
    try {
      ...
      if (foundService && foundWriteChar && foundReadChar) {
        ...
        if (writeCharacteristic != null && readCharacteristic != null) {
          ...
          print('❌ 未能找到必要特征，连接失败');
          print(
              '写入特征: ${writeCharacteristic != null}, 读取特征: ${readCharacteristic != null}');
  ...
  Future<void> sendFMFrequencyCommand(double frequency) async {
    ...
    if (frequencyInt >= 1000) {
      print('频率范围: 100MHz以上');
      print(
          '编码: byte2=0x${byte2.toRadixString(16).padLeft(2, '0')} (表示100MHz以上), byte3=0x${byte3.toRadixString(16).padLeft(2, '0')} (十位), byte4=0x${byte4.toRadixString(16).padLeft(2, '0')} (小数位)');
    } else {
      print('频率范围: 100MHz以下');
      print(
          '编码: byte2=0x${byte2.toRadixString(16).padLeft(2, '0')} (百位), byte3=0x${byte3.toRadixString(16).padLeft(2, '0')} (十位), byte4=0x${byte4.toRadixString(16).padLeft(2, '0')} (小数位)');
    }
  ...
  Future<void> sendMicModeCommand(int value) async {
    ...
    print('Mic mode值说明 - 0: NORMAL, 1: GIRL, 2: MAN');

    // 确保Mic mode值在有效范围内
    if (value < 0 || value > 2) {
      print('❌ 无效的Mic mode值: $value，值应为 0-2');
      throw ArgumentError('Mic mode值必须在 0-2 范围内');
    }
    ...
    final List<int> packet = [
      0xBE, // 包头
      0x14, // Mic mode指令
      value // Mic mode值 (0:NORMAL, 1:GIRL, 2:MAN)
    ];
  ...
  // 新增：发送Reverb控制指令
  Future<void> sendReverbCommand(int value) async {
    print('发送Reverb指令: $value');
    print('Reverb值范围: 0-16');

    // 确保Reverb值在有效范围内
    if (value < 0 || value > 16) {
      print('❌ 无效的Reverb值: $value，值应为 0-16');
      throw ArgumentError('Reverb值必须在 0-16 范围内');
    }
  ...
  // 新增：发送Echo控制指令
  Future<void> sendEchoCommand(int value) async {
    print('发送Echo指令: $value');
    print('Echo值范围: 0-16');

    // 确保Echo值在有效范围内
    if (value < 0 || value > 16) {
      print('❌ 无效的Echo值: $value，值应为 0-16');
      throw ArgumentError('Echo值必须在 0-16 范围内');
    }
  ...
  // 新增：发送Priority控制指令
  Future<void> sendPriorityCommand(int value) async {
    print('发送Priority指令: $value');
    print('Priority值说明 - 0: 关, 1: 开');

    // 确保Priority值在有效范围内
    if (value < 0 || value > 1) {
      print('❌ 无效的Priority值: $value，值应为 0 或 1');
      throw ArgumentError('Priority值必须在 0-1 范围内');
    }

    // 构造指令包: [包头, 指令, Priority值]
    final List<int> packet = [
      0xBE, // 包头
      0x10, // Priority指令
      value // Priority值 (0:关, 1:开)
    ];

    print('Priority指令包: $packet');
    print(
        '十六进制: ${packet.map((b) => '0x${b.toRadixString(16).padLeft(2, '0')}').join(', ')}');

    await sendCommand(packet, maxRetries: 3);
    print('✅ Priority指令发送完成');
  }

  // 新增：发送FBX控制指令
  Future<void> sendFbxCommand(int value) async {
    print('发送FBX指令: $value');
    print('FBX值说明 - 0: 关, 1: 开');

    // 确保FBX值在有效范围内
    if (value < 0 || value > 1) {
      print('❌ 无效的FBX值: $value，值应为 0 或 1');
      throw ArgumentError('FBX值必须在 0-1 范围内');
    }
    ...
    if (writeCharacteristic != null) {
      try {
        await writeCharacteristic!
            .write(_READ_ALL_STATE_COMMAND, withoutResponse: false);
  ...
  void _parseDeviceResponse(List<int> data) {
    ...
    // 处理设备通知数据 (0xEB开头的18字节数据)
    if (data[0] == 0xEB && data.length >= 18) {
    ...
    if (command == 0x00 && data.length >= 18) {
      // 假设：
      ...
      print(
          '✅ Full state parsed and updated. Volume: $volume, MusicVB: $musicVB, 3D: $sound3D, MusicEQ: $musicEQ, MicVol: $micVol, MicEQ: $micEQ');
    } else {
      // 处理其他类型的指令
      print(
  ...
  void _handleDeviceNotification(List<int> data) {
    ...
    try {
      ...
      // 字节0: 0xEB (包头)
      // 字节1: 0x00 (指令)
      // 按顺序分别是：Volume、X.BASS、3D、MusicEQ、KARAOKE、MicEQ、输入源、Music Control、FM频点(4个字节)、Reverb
      int volume = data[2]; // Volume (0-32) - 字节2
      int xBassValue = data[3]; // X.BASS (0-3) - 字节3
      // 将X.BASS的值映射为字符串：0->OFF, 1->1, 2->2, 3->3
      String xBass = xBassValue == 0 ? 'OFF' : xBassValue.toString();
      int sound3D = data[4]; // 3D (0-8) - 字节4
      int musicEQ = data[5]; // MusicEQ (0-8) - 字节5
      int karaoke = data[6]; // KARAOKE (0-32) - 字节6
      int micEQ = data[7]; // MicEQ (0-8) - 字节7
      int inputSource = data[8]; // 输入源 (0:BT, 1:AUX, 2:USB/SD, 3:FM) - 字节8
      int musicControl = data[9]; // music control播放暂停 (0:暂停, 1:播放) - 字节9

      // 解析Mic mode (0:NORMAL, 1:GIRL, 2:MAN)
      String micMode = _mapMicModeToString(micEQ);

      // FM频点(4个字节)
      int fmByte1 = data[10]; // 第1个字节 - 字节10
      int fmByte2 = data[11]; // 第2个字节 - 字节11
      int fmByte3 = data[12]; // 第3个字节 - 字节12
      int fmByte4 = data[13]; // 第4个字节 - 字节13

      // 将FM频点转换为频率
      double fmFrequency =
          _parseFMFrequency(fmByte1, fmByte2, fmByte3, fmByte4);

      // Reverb (0-16) - 字节16
      int reverb = data[16]; // Reverb (0-16) - 字节16

      // Echo (0-16) - 字节17
      int echo = data[17]; // Echo (0-16) - 字节17

      // FBX (0-16) - 字节18
      int fbx = data[18]; // FBX (0-16) - 字节18

      // PRIORITY (0-1) - 字节14
      int priority = data[14]; // PRIORITY (0-1) - 字节14

      // 打印调试信息
      print(
          '📻 FM频点数据: byte1=$fmByte1, byte2=$fmByte2, byte3=$fmByte3, byte4=$fmByte4');
      print('📻 解析后的FM频率: $fmFrequency MHz');
      print('📻 当前输入源: $inputSource (0:BT, 1:AUX, 2:USB/SD, 3:FM)');
      print('📻 Reverb: $reverb');
      print('📻 Echo: $echo');
      print('📻 FBX: $fbx');
      ...
      Map<String, dynamic> state = {
        ...
        'reverb': reverb.clamp(0, 16),
        'echo': echo.clamp(0, 16),
        'fbx': fbx.clamp(0, 16),
        'priority': priority.clamp(0, 1),
        'timestamp': DateTime.now().millisecondsSinceEpoch,
  ...
  // 解析FM频率（4字节数据）
  double _parseFMFrequency(int byte1, int byte2, int byte3, int byte4) {
    // FM频率编码规则（4字节数据）：
    // byte1: 百位数字（8或9）或0x0A（表示100MHz以上）
    // byte2: 十位数字
    // byte3: 个位数字
    // byte4: 小数位数字

    print('🔍 FM频率解析: byte1=$byte1, byte2=$byte2, byte3=$byte3, byte4=$byte4');

    // 计算频率整数值
    int frequencyInt;

    // byte1作为百位数字，byte2作为十位，byte3作为个位，byte4作为小数位
    frequencyInt = byte1 * 100 + byte2 * 10 + byte3;
    print('🔍 频率整数部分: $byte1 * 100 + $byte2 * 10 + $byte3 = $frequencyInt');

    // 将整数值转换为频率（除以10，保留一位小数）
    // 使用byte4作为小数位
    double frequency = frequencyInt.toDouble() + byte4 / 10.0;
    print('🔍 频率计算: $frequencyInt + $byte4 / 10.0 = $frequency MHz');

    // 确保频率在有效范围内
    double clampedFrequency = frequency.clamp(87.5, 108.0);
    if (frequency != clampedFrequency) {
      print('⚠️ 频率超出范围，已限制: $frequency -> $clampedFrequency');
    }
  ...
  // 将Mic mode值转换为字符串
  String _mapMicModeToString(int value) {
    switch (value) {
      case 0:
        return 'Normal';
      case 1:
  ...
  void _handleSyncCommand(List<int> data) {
    ...
    try {
      ...
      int xBass = data.length > 3 ? data[3] : 0;
      int sound3D = data.length > 4 ? data[4] : 0;
      int musicEQ = data.length > 5 ? data[5] : 0;
      int karaoke = data.length > 6 ? data[6] : 0;
      int micEQ = data.length > 7 ? data[7] : 0;
      int inputSource = data.length > 8 ? data[8] : 0;
      int musicControl = data.length > 9 ? data[9] : 0;

      // 数据验证
      volume = volume.clamp(0, 32);
      xBass = xBass.clamp(0, 3);
      sound3D = sound3D.clamp(0, 8);
      musicEQ = musicEQ.clamp(0, 8);
      karaoke = karaoke.clamp(0, 32);
      micEQ = micEQ.clamp(0, 8);
      inputSource = inputSource.clamp(0, 3);
      musicControl = musicControl.clamp(0, 1);

      print(
          '🔄 设备主动同步 - 音量: $volume, X.BASS: $xBass, 3D: $sound3D, 音乐EQ: $musicEQ, KARAOKE: $karaoke, 麦克风EQ: $micEQ, 输入源: $inputSource, 播放状态: $musicControl');
  ...
  void _handleFMFrequencyChange(List<int> data) {
    ...
    print(
        '📻 Device FM frequency change: 0x${byte1.toRadixString(16)}, 0x${byte2.toRadixString(16)}, 0x${byte3.toRadixString(16)}, 0x${byte4.toRadixString(16)}');
  ...
  Future<bool> rediscoverServices() async {
    ...
    try {
      ...
      // 重新发现服务
      List<BluetoothService> services =
          await connectedDevice!.discoverServices(timeout: 10000);
